...
function single_machine_release_LMax(
    ...
    if false # possible_sequence !== nothing
    ...
    while !isempty(stack)
        ...
        if length(node.jobs) == 0
            ...
            # println("1 new sequence found: $(map(x->x.index,node.jobsOrdered)), lb = $(node.lowerBound)")
            if node.lowerBound < upperBound
                upperBound = node.lowerBound
                minNode = node
            end
        elseif node.lowerBound < upperBound
            ...
            for i in node.jobs
                ...
                # println("nodeCopy = $(map(x->x.index,nodeCopy.jobsOrdered))")
                ...
                microruns += 1
                if false # possible_sequence ≢  nothing
                    # println("2 new sequence found: $(map(x->x.index,[nodeCopy.jobsOrdered; possible_sequence])), lb = $(node.lowerBound)")
...
single_machine_release_LMax2(p::Vector{Int64}, r::Vector{Int64}, d::Vector{Int64}) = single_machine_release_LMax2(p, r, d, nothing)

function single_machine_release_LMax2(
    p::Vector{Int64},
    r::Vector{Int64},
    d::Vector{Int64},
    yield_ref::Union{Nothing,Ref{Float64}}
    ...
    node.lowerBound, possible_sequence = single_machine_release_LMax_pmtn2([JobData(p[i], r[i], d[i], i, nothing) for i in node.jobs], node.jobsOrdered, node.time)
    ...
    while !isempty(stack)
        ...
        elseif node.lowerBound < upperBound
            ...
            for i in node.jobs
                ...
                # jeśli otrzymany harmonogram possible_sequence nie jest równy nothing, to znaczy,
                # że optymalny haronogram jest bez wywłaszczeń, więc jest to węzeł końcowy.
                nodeCopy.lowerBound, possible_sequence = single_machine_release_LMax_pmtn2([JobData(p[i], r[i], d[i], i, nothing) for i in nodeCopy.jobs], nodeCopy.jobsOrdered, nodeCopy.time)
                microruns += 1
                if possible_sequence ≢  nothing
                    # println("2 new sequence found: $(map(x->x.index,[nodeCopy.jobsOrdered; possible_sequence])), lb = $(node.lowerBound)")
                    nodeCopy.jobsOrdered = [nodeCopy.jobsOrdered; possible_sequence]
...
preemptive EDD
"""
function single_machine_release_LMax_pmtn2(
    jobs::Vector{JobData},
    jobsOrdered::Vector{JobData},
    startTime::Int64=0
)
    ...
    while !isempty(deadlineQueue)
        jobToProceed = dequeue!(deadlineQueue)
        # println("jobToProceed = $jobToProceed")
        # flaga informująca, czy zadanie jobToProceed zostało przerwane przez jakieś inne zadanie,
        # które zostało dostępne w czasie wykonywania jobToProceed, a ma szybszy deadline
        jobPreempted = false
...
function test()
    ...
    println(result)
    result2 = single_machine_release_LMax2(newP,newR,newD)
    println(result2)
    @show newD
end
